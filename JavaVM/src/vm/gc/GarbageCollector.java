...
public class GarbageCollector {
     ...
     *      5.并发清理
     *      6.并发重置
     *  concurrent mode failure：指的是在预清理的期间发生内存空间不足的情况，进而触发full gc
     *  避免cmf：增加新生代大小，将老年代gc的阈值调小，full gc后做空间压缩
     *
     *
     * 新生代&老年代：
     ...
     *  g1的过程：
     *     初始标记（STW），标记从根节点可达的对象。该过程伴随一次minor gc，因此是独占式的(young)
     *     根区域扫描，该过程会扫描由survivor区域可达的老年代区域（也就是survivor区的对象引用的老年代对象所处的区域）
     *     并发标记，发现整个堆中的存活对象，
     *     重新标记（STW），对标记结果进行修正（采用SATB算法）
     *     独占清理（STW），计算存活对象，释放region
     *     并发清理，识别并清理完全空闲的区域
     *     复制对象：young或者mix
     *
     *  Remember Set： 每个region对应一个RS用来记录region中对象的引用关系
     *  Collection Set：会被收集的region的集合
     **/
